...
router.get("/", authenticate, requirePermission("dispatch.read"), async (req, res, next) => {
  try {
...
router.post("/", authenticate, requirePermission("dispatch.create"), validate(dispatchCreateSchema), async (req, res, next) => {
  try {
    ...
    await audit({ userId: req.user.id, action: "create", entityType: "dispatch", entityId: dispatch.id, newValue: { orderId, warehouseId } });
    res.status(201).json({ success: true, data: dispatch });
  } catch (error) { next(error); }
});

router.get("/:id", authenticate, requirePermission("dispatch.read"), async (req, res, next) => {
  try {
...
router.patch("/:id/scan", authenticate, requirePermission("dispatch.scan"), validate(dispatchScanSchema), async (req, res, next) => {
  try {
    ...
    await audit({ userId: req.user.id, action: "scan", entityType: "dispatch", entityId: req.params.id, newValue: { barcodeId, scanned } });
